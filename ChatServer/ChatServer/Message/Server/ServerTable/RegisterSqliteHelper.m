...
- (BOOL)insertIntoTableWithAccount:(NSString *)account password:(NSString *)password state:(int)state userId:(uint64_t)userId{
    __block BOOL ret = NO;
    [[[ChatServerClient server].dbHelper databaseQueue] inDatabase:^(FMDatabase *db) {
        NSString *sql = [NSString stringWithFormat:@"insert into %@ (account, password, state, userid) values ('%@', '%@', '%d', '%lld');", [self tableName], account, password, state, userId];
        ret = [db executeUpdate:sql];
    }];
    return ret;
}
...
- (BOOL)updateTableWithAccount:(NSString *)account password:(NSString *)password state:(int)state{
    ...
    [[[ChatServerClient server].dbHelper databaseQueue] inDatabase:^(FMDatabase *db) {
        NSString *sql = [NSString stringWithFormat:@"update %@ set%@%@%@ where account = '%@';", [self tableName], chanegePassword, and, changeState, account];
        ret = [db executeUpdate:sql];
    }];
    return ret;
}
